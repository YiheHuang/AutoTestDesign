...
## 任务
对给定的标准需求，分析每个输入字段的等价类，并为每个等价类生成测试用例。test_cases 必须非空数组，每个等价类至少对应一个测试用例。
...
## 任务
对给定的标准需求，识别数值/长度边界，生成6点边界值测试。test_cases 必须非空数组，每个边界点至少对应一个测试用例。
...
## 任务
对给定的标准需求，从条件中构建判定表，化简后生成测试用例。test_cases 必须非空数组，每条化简规则至少对应一个测试用例。
